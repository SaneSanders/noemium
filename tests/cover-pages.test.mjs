...
test('cover home is the night hero, not the old floor', () => {
  ...
  assert.match(html, /nema-final4-cut\.png/);
  ...
  assert.equal(existsSync(new URL('dist/og-cover.png', root)), true);
...
test('mcp page is a graded shelf, not a registry scrape', () => {
  ...
  assert.match(html, /Firecrawl MCP/);
  assert.match(html, /Sequential Thinking/);
  assert.match(html, /Everything MCP/);
